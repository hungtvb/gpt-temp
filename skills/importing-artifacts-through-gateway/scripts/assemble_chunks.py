...
def load_chunk(path: Path) -> dict[str, Any]:
    ...
    if missing:
        raise ValueError(f"{path}: missing chunk fields: {sorted(missing)}")
    ...
    if len(data) != length:
        raise ValueError(f"{path}: expected {length} bytes, decoded {len(data)}")

    actual = sha256(data)
    expected = str(chunk["chunkSha256"]).lower()
    if actual != expected:
        raise ValueError(
            f"{path}: chunk SHA-256 mismatch: expected {expected}, got {actual}"
        )
...
def main() -> int:
    ...
    chunks = sorted((load_chunk(path) for path in args.chunks), key=lambda x: x["offset"])
    ...
    for chunk in chunks:
        if chunk["totalSize"] != total_size:
            raise ValueError(f"{chunk['_source']}: inconsistent totalSize")
        if chunk["fullSha256"] != full_sha:
            raise ValueError(f"{chunk['_source']}: inconsistent fullSha256")
        if chunk["offset"] != expected_offset:
            raise ValueError(
                f"{chunk['_source']}: expected offset {expected_offset}, got {chunk['offset']}"
            )
    ...
    if len(assembled) != total_size:
        raise ValueError(
            f"incomplete artifact: expected {total_size} bytes, assembled {len(assembled)}"
        )

    actual_full_sha = sha256(assembled)
    if actual_full_sha != full_sha:
        raise ValueError(
            f"full SHA-256 mismatch: expected {full_sha}, got {actual_full_sha}"
        )
